# Incremental X jog in current work plane (G54). Positive = +X in work coords.
...
Write-Host "Using $Com  (work X += $DeltaMm mm @ F$Feed)"
...
function Cmd([System.IO.Ports.SerialPort]$p, [string]$line, [int]$waitMs = 120000) {
  $p.DiscardInBuffer()
...
function Wait-GrblIdle([System.IO.Ports.SerialPort]$p, [int]$timeoutMs = 180000) {
  ...
  throw 'Timeout waiting for GRBL Idle after jog.'
}
...
$xLine = ('G1 X{0:F4} F{1:F0}' -f $DeltaMm, $Feed)
Write-Host ">> $xLine"
$r = Cmd $port $xLine 180000
if ($r -match '(?i)error:\d+') { Write-Host $r; throw "GRBL error on: $xLine" }

Write-Host 'Waiting for jog to finish (Idle)...'
...
Write-Host 'Done (incremental X in work coords).'
